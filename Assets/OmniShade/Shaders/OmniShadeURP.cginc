...
half3 AdditionalLightsFrag(half3 pos_world, half3 nor_world, half4 pos_clip, half _DiffuseWrap, half _DiffuseBrightness, half _DiffuseContrast, uint meshRenderingLayers) {
	half3 col_diffuse = 0;
	#if _ADDITIONAL_LIGHTS
		...
		LIGHT_LOOP_BEGIN(lightsCount)
			...
			#endif
			{
				...
				diff *= AdditionalLightsShadow(lightIndex, pos_world, light);
...
half3 AdditionalLightsVert(half3 pos_world, half3 nor_world, half4 pos_clip, half _DiffuseWrap, half _DiffuseBrightness, half _DiffuseContrast, uint meshRenderingLayers) {
	half3 col_diffuse = 0;
	#if _ADDITIONAL_LIGHTS
		uint lightsCount = GetAdditionalLightsCount();
		#if USE_FORWARD_PLUS
...
half3 AdditionalLightsVert(half3 pos_world, half3 nor_world, half4 pos_clip, half _DiffuseWrap, half _DiffuseBrightness, half _DiffuseContrast, uint meshRenderingLayers) {
	return AdditionalLightsFrag(pos_world, nor_world, pos_clip, _DiffuseWrap, _DiffuseBrightness, _DiffuseContrast, meshRenderingLayers);
}
